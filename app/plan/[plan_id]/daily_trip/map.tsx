...
const MAP = () => {
    ...
    const goToNextPlace = () => {
        if (markerPlaces.length > 0) {
            const newIndex =
                currentPlaceIndex < markerPlaces.length - 1
                    ? currentPlaceIndex + 1
                    : 0;
            setCurrentPlaceIndex(newIndex);
        }
    ...
    const fetchTripsDetails = async () => {
        ...
        const allDates = extractDates(
            tripsDetail.start_date,
            tripsDetail.end_date
        );
        setDates(allDates);
