...
export default function SelectTime() {
  // NOTE: we need BOTH plan_id and vote_id for the dynamic segments
  ...
  const goNext = () => {
    ...
    // IMPORTANT: Use the file-system pattern for dynamic segments and supply params separately.
    router.push({
      pathname:
        "/plan/[plan_id]/daily_trip/(modals)/add_vote/[vote_id]/vote_place",
      params: {
        plan_id: String(plan_id),
        vote_id: String(vote_id), // comes from the current route
  ...
  const box =
    "mt-2 h-16 w-full rounded-2xl border border-[#D9D9D9] bg-white items-center justify-center";
