...
const ResultVoteEvent = () => {
    ...
    const fetchVoteDetail = (vote_id: string, plan_id: string) => {
        const result = mockActivityVoteEvents.find(
            vote =>
                vote.id === parseInt(vote_id) &&
                vote.trip_id === parseInt(plan_id)
        );
    ...
    const getVoteCounts = () => {
        ...
        votes.forEach(vote => {
            if (vote.event_id) {
                voteCounts[vote.event_id] =
                    (voteCounts[vote.event_id] || 0) + 1;
            }
        });
        return voteCounts;
    };
    ...
    return (
        <ScrollView className="flex-1 bg-white">
            ...
            <View className="px-6 mt-4">
                ...
                {/* Title Section */}
                <View className="bg-white rounded-lg border border-gray_border p-4 mb-4">
                    <Text className="text-xl font-bold text-black mb-3 ml-4">
                        Title
                    </Text>
                ...
                <View className="flex-row flex-wrap justify-between mb-6">
                    {mockTransportationOptions.map(option => {
                        ...
                        const isMostVoted = mostVotedOptions.includes(
                            option.id
                        );
