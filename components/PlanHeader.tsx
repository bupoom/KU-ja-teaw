...
function PlanHeader({ planId }: { planId: string }) {
  ...
  const navigateToTab = (tab: string) => {
    // ถ้าหน้าเดิมอยู่แล้ว → ไม่ต้อง push ซ้ำ
    if (
      (tab === "overview" && pathname === `/plan/${planId}`) ||
      (tab === "daily_trip" && pathname.startsWith(`/plan/${planId}/daily_trip`)) ||
      (tab === "group" && pathname.startsWith(`/plan/${planId}/group`))
    ) {
      return;
    }
  ...
  return (
    <View className="bg-white">
      ...
      {/* Custom Tabs */}
      <View className="flex-row bg-white px-4 py-4 border-b border-gray_border">
        <TouchableOpacity
        ...
        >
          <Text
            className={`text-base font-medium ${
              activeTab === "daily_trip" ? "text-white" : "text-gray-600"
            }`}
          >
            Daily Trip
          </Text>
        ...
        >
          <Text
            className={`text-base font-medium ${
              activeTab === "group" ? "text-white" : "text-gray-600"
            }`}
          >
            Group
          </Text>
