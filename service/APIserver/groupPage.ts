...
export const get_trip_member = async (
    trip_id: number
): Promise<TripMember[]> => {
    try {
        console.log("fetching trip member");
        const response = (await apiClient.get(
            `${endpoints.member.all_member}/${trip_id}`
        )) as { data: any[] };
...
export const delete_mem = async (
    ...
    try {
        ...
        const response = await apiClient.delete(
            `${endpoints.member.all_member}/${trip_id}/${collab_id}`
        );
    } catch (error) {
        console.error("Response data:", error);
        throw error;
...
export const search_user = async (
    ...
    try {
        ...
        const response = (await apiClient.get(
            `${endpoints.search.user}/${username}/${trip_id}`
        )) as { data: any[] };
...
export const invite_user = async (
    ...
    try {
        ...
        const response = await apiClient.post(`/api/trips/${trip_id}/invite`, {
            name: `${username}`,
        }); // อาจจะมีปัญหาที่หน้า page เพราะไม่ returnว่า success มั้ย
    } catch (error) {
        console.error("Response data:", error);
        throw error;
    }
